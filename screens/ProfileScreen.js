...
useEffect(() => {
 ...
 try {
  ...
  // The backend (routes/profile.js GET /:user_id) should now return decrypted data
...
// ===================================
// 🔄 TOGGLE HIDE INFO (CRITICAL LOGIC)
// ===================================
const toggleHideInfo = async (value) => {
 // Optimistically update the UI switch
 setIsHidden(value);
 ...
 if (data.success && data.user) {
  ...
  // Use the complete, UNENCRYPTED user object returned by the backend's /privacy endpoint.
